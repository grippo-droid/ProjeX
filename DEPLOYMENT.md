...
This guide details how to deploy the ProjeX.
...
2.  **Connect GitHub**: Select your `ProjeX` repository.
3.  **Configure Settings**:
    - **Name**: `projex-api` (or similar)
...
> [!NOTE]
> Wait for the deployment to finish. Once done, copy the **onrender.com URL** (e.g., `https://projex-api.onrender.com`).
...
2.  **Select Repository**: Import `ProjeX`.
3.  **Configure Project**:
...
4.  **Environment Variables**:
    ...
    - `REACT_APP_API_URL`: Paste the **Render Backend URL** from the previous step.
      *(Example: `https://projex-api.onrender.com` - **do not** include a trailing slash)*
...
1.  **Update Backend CORS (Optional/Recommended)**:
    - Once the frontend is live on Vercel, copy the **Vercel URL** (e.g., `https://projex-frontend.vercel.app`).
